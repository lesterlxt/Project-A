...
- lookup_fund: 查询具体基金（"查基金000001""帮我看看110011""000001怎么样"）
- show_copy: 查看已生成文案的详情（"看看话术""完整文案"）
- help: 帮助/问候/不知道干什么/你好

返回格式：
{"action": "<intent>", "topic": "<提取的主题>", "channel": "<提到的渠道>", "fund_code": "<6位数字基金代码>", "top_k": 5}

渠道：招商银行/工商银行/建设银行/农业银行/中国银行。未提及时默认"招商银行"。
基金代码是6位纯数字。top_k 默认3，用户说具体数量时提取（如"推荐5只"则 top_k=5）。
...
class FeishuBotService:
    ...
    async def _process_message(
        ...
        try:
            if action == "list_hotspots":
                ...
                await self._handle_lookup_fund(receive_id, receive_id_type, code)
    ...
    def _fallback_classify(text: str) -> dict[str, Any]:
        ...
        if any(kw in t for kw in ["查", "看看", "详情"]):
            code = FeishuBotService._extract_fund_code(text)
            if code:
                return {"action": "lookup_fund", "fund_code": code, "topic": "", "channel": "招商银行", "top_k": 3}
    ...
    async def _handle_lookup_fund(
        self, receive_id: str, receive_id_type: str, fund_code: str,
    ) -> None:
        """Look up a fund by code and send a detail card."""
        if not fund_code or len(fund_code) != 6:
            self._send_text(receive_id, receive_id_type, "请提供6位数字的基金代码，比如「查基金 000001」。")
            return

        self._send_text(receive_id, receive_id_type, f"正在查询基金 {fund_code}，请稍候...")

        try:
            funds = self.fund_loader.load()
            match = next((f for f in funds if f.fund_code == fund_code), None)

            if match is None:
                self._send_text(receive_id, receive_id_type, f"未找到基金代码 {fund_code}，请确认代码是否正确。")
                return

            # Convert Fund to RecommendedFund-like dict for card builder
            fund_data = self._fund_to_card_data(match)
