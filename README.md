...
- **易方达自有基金池**：候选基金限定为易方达自有产品，分析前 Dashboard 同时展示易方达官网基金超市样本作为官方产品页对照。
...
- `efunds.com.cn/lm/jjcp/` 用于展示易方达官网基金超市样本对照；本地候选基金池也已收窄为易方达自有基金，详情字段仍通过东方财富 / 天天基金公开接口补全。

需要注意：

- `funds.db` 是本地 SQLite 缓存，已被 `.gitignore` 忽略。
- 默认基金池同步逻辑是：先读取公开基金代码列表，再强制保留基金名称以“易方达”开头的自有产品，默认同步约 789 只进入本地候选基金池，并尝试增强其中最多 500 只的经理、收益、持仓和风险字段。
